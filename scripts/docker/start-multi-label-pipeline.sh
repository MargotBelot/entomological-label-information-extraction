#!/bin/bash
#
# Multi-Label Pipeline Starter Script
#
# This script automatically:
# 1. Ensures Docker is running
# 2. Cleans up any previous pipeline runs
# 3. Starts the multi-label processing pipeline
...
COMPOSE_FILE="${PROJECT_ROOT}/pipelines/multi-label-docker-compose.yaml"

echo "Starting Multi-Label Entomological Label Processing Pipeline"
echo "======================================================"
...
create_directories() {
    ...
    mkdir -p "${PROJECT_ROOT}/data/MLI/input"
    mkdir -p "${PROJECT_ROOT}/data/MLI/output"
    echo "SUCCESS: Directories created"
}

# Function to validate input directory
validate_input() {
    local input_dir="${PROJECT_ROOT}/data/MLI/input"
...
main() {
    ...
    if docker-compose -f "$COMPOSE_FILE" up --build; then
        ...
        echo "Consolidated results: ${PROJECT_ROOT}/data/MLI/output/consolidated_results.json"
    else
