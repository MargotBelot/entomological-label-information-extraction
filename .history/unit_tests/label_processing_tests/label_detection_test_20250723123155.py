...
class TestSegmentationCropping(unittest.TestCase):
    ...
    def test_crops(self):
        ...
        create_crops(os.path.join(os.path.dirname(__file__), "../testdata/uncropped"), df, out_dir=Path("check_crops"))
    ...
    def tearDown(self):
        ...
        """
        if os.path.exists("check_crops"):
            for f in glob.glob("check_crops/*"):
                try:
                    ...
                    continue
            try:
                os.rmdir("check_crops")  # Remove the empty directory
            except OSError as e:
